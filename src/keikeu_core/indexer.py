...
def _direct_markdown_names(directory_fd: int) -> list[str]:
...
def _rebuild_index_at(vault: Path, root_fd: int) -> dict[str, object]:
    ...
    try:
        for name in _direct_markdown_names(cache_fd):
            path = vault / "cache" / name
            try:
                data, _identity = _read_regular_bytes_at(cache_fd, name, path)
                papers.append(_paper_entry(vault, path, parse_paper_bytes(data)))
            except (OSError, ValueError) as exc:
                errors.append(
                    {"path": str(path.relative_to(vault)), "reason": str(exc)}
                )
    finally:
        os.close(cache_fd)
...
def _entry_is_current_at(
    ...
    try:
        ...
        relative = _direct_paper_relative_path(vault, candidate, ("cache",))
        path = vault / relative
        data, _identity = _read_regular_bytes_at(cache_fd, relative.name, path)
...
def _index_entries_are_safe_at(
    ...
    return (
        ...
        and all(_entry_is_current_at(vault, cache_fd, entry) for entry in papers)
