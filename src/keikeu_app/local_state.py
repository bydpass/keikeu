"""Disposable per-device state for the once-per-local-day start card.

Paper Markdown remains the source of truth.  This app-layer JSON file stores
only the last local calendar date whose built-in start card was claimed.
Missing or malformed state is treated as no claim and never affects a Vault.
"""
...
def claim_daily_card(
    ...
    """Atomically record ``local_date`` when today's card has not been shown.

    Returns ``True`` only when the caller should display the card.  The date is
    written before returning so a crash or rapid relaunch cannot show it twice.
    """
