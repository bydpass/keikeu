...
| Road v0.5 | CP7 passed by advance YOLO; commit pending | CP0 is committed at `11c2149`; CP1 at `a2ff5cb`; CP2 at `30b54ac`; CP3 at `c7b9caf`; CP4 at `371f4e8`; CP6 at `4753d1d`. The [CP6 report](acceptance/road-v0-5/cp6-dogfood/report.md) records accepted product dogfood. The [CP7 report](acceptance/road-v0-5/cp7-fixed-sidebars/report.md) records fixed context rails, bounded Flashcard-list scrolling, hidden scrollbar chrome, no-overscroll CSS, two desktop sizes, synthetic data, and current-source Tauri evidence. CP7 is not committed; Road closeout waits for that separate authorization and commit. |
...
- **Views:** [design](design/design.html) and [interaction](design/interaction.html) describe the implemented Road v0.5 visual and interaction system; [architecture](architecture/architecture.html) describes the current runtime. CP6 product acceptance and the advance-YOLO CP7 engineering gate are complete; CP7 commit and Road closeout remain separate.
...
Road v0.4 is complete. Road v0.5 CP1 is committed as `a2ff5cb`; CP2 as `30b54ac`; CP3 as `c7b9caf`; CP4 as `371f4e8`; CP6 as `4753d1d`. CP5 engineering and isolated synthetic QA passed by advance YOLO. CP6 independent Agent dogfood passed developer acceptance with no observed P0/P1, three rechecked Round 1 fixes, and a repaired P2 Vault switch. CP7 fixed the three page context rails, nested Flashcard scrolling, hidden scrollbar chrome, and overscroll boundaries; its engineering and current-source Tauri gate passed by advance YOLO, but its commit is not authorized yet. Commit, Road snapshot, tag, push, signing, distribution, and any real-Vault operation remain separate developer decisions.
