...
| Road v0.7 | Product and implementation complete; Git closeout pending | CP0 is `fb52b55`; CP1 is `8019969`; CP2 is `55d45fc`; CP3 is `55b5313`; CP4 is `6f69310`; CP5 is `3259c42`; CP6 is `e22b691`; CP7 is `1e17cea`. Current branch `ui/cp8-v07-responsive-navigation` completed CP8 implementation, full engineering/Figma evidence and physical-keyboard macOS IME Gate D. The developer explicitly accepted CP8 and declared Road v0.7 complete on 2026-08-26. The [CP8 acceptance report](acceptance/road-v0-7/cp8-responsive-navigation/report.md) keeps engineering, developer QA and Git conclusions separate. HEAD remains the CP7 checkpoint `1e17cea`; CP8 checkpoint commit, Road snapshot, real Vault, push, tag and release are not authorized. |
| Road v0.8 | Direction recorded; not started | Earliest start is 2026-08-28. Scope direction is packaging plus Alpha release. No branch, plan, signing, package, distribution, remote write or release action has started; Road v0.7 Git closeout remains the required linear boundary. |
...
git:       HEAD 1e17cea (CP7) → accepted CP8 working tree remains uncommitted
road:      product and implementation complete; checkpoint / snapshot / tag remain separate
next:      after authorized v0.7 Git closeout, Road v0.8 packaging + Alpha may start no earlier than 2026-08-28
```
...
- **Authority:** [SPEC](SPEC.md) owns the accepted CP8 product boundary; the [Paper v4 design](design/road-v0-6-paper-v4-design.md) owns unchanged grammar/protocol; the [App Shell design](design/road-v0-7-app-shell-design.md) owns CP6 history and the accepted CP7/CP8 matrices; the Chinese [plan](../PLAN_road_v0_7.md) owns Gate order. [RULES](RULES.md) owns implementation discipline; [AGENTS](../AGENTS.md) owns agent procedure. Runtime facts come from [`src/`](../src/) and [`tests/`](../tests/).
- **Views:** [design](design/design.html), [interaction](design/interaction.html), and [architecture](architecture/architecture.html) mirror the accepted CP8 presentation on unchanged Paper v4/Index v4/protocol v2. They do not independently prove Gate D, Road closeout or release.
...
Road v0.4, Road v0.5, and Road v0.6 are complete. The developer declared Road v0.7 product and implementation work complete on 2026-08-26 after CP0–CP8 passed. CP7 remains the latest committed checkpoint at `1e17cea`, while the accepted CP8 working tree is uncommitted; checkpoint, Road snapshot, push, tag and release remain separate, unperformed decisions. Paper v4/Index v4/protocol v2 and the complete-folder lifecycle remain unchanged; no real-Vault operation is authorized. Road v0.8 is not started: its direction is packaging plus Alpha release, no earlier than 2026-08-28 and only after an explicit plan and clean linear Git boundary.
