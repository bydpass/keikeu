...
```text
current: editable card-page Paper → saved and retrievable Paper; Road v0.6 archived after first-author acceptance
next:    choose later Road work separately; tag, push and release remain unperformed
```
...
- **Working material:** Road v0.6 CP6 is `02d8ed5`; CP7 passed its actual author Gate at exact checkpoint `18a1024` on `test/cp7-real-author-gate`. The later `3df9760` repository-policy commit is not a Road checkpoint. The old invited-Alpha release target is revoked; [Road v0.5 working material](design/working-materials.md) is historical.
