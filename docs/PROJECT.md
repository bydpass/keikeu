...
Updated: 2026-09-04 (plan/documentation refactor only; no runtime or device re-verification)
...
| Road v0.8 | Revised direction documented; software CP0 seed not approved; not started | The developer approved the [plan refactor](../PLAN_road_v0_8.md) on 2026-09-04: retain Vue/Tauri, deliver iPhone core creation and mandatory Mac iCloud synchronization, then independently achieve desktop parity and retire the Python product runtime before external Alpha. [ADR-0009](architecture/decisions/0009-unified-rust-core-transition.md) records this target, not a current implementation. First independently fix and reverify the close guard; CP0 freezes contracts, CP1 proves Apple feasibility, CP2 builds Rust Core, CP3 delivers bilingual iPhone local creation, CP4 integrates iCloud, CP5 accepts the candidate. No engineering stage has started. |
...
next:      independently fix and reverify the close guard, then approve the v0.8 CP0 seed; contracts freeze inside CP0, Apple probes start at CP1
```

**2026-08-26 implementation deviation:** The accepted contract keeps App-root pending durable intent under normal-close protection until reconciliation or explicit abandonment. Source stores the intent in `App.vue`, but `PaperView.vue` owns `registerWindowCloseGuard()`; runtime-blocked screens therefore unmount the only Tauri guard, and Library/Vault durable intents have no equivalent App-root guard. This audit changed no production code or real Vault. The deviation remains an independent pre-Road blocker until a narrow fix is assigned and focused Paper/Library/Vault/blocked-screen tests reverify it; it must not be absorbed into a Road v0.8 checkpoint. See [SPEC](SPEC.md), the [Paper v4 design](design/road-v0-6-paper-v4-design.md), and the accepted [App Shell design](design/road-v0-7-app-shell-design.md); source and tests below remain runtime fact.
