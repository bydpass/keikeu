...
| Road v0.7 | CP5 passed; CP6 next | CP0 is `fb52b55`; CP1 is `8019969`; CP2 is `55d45fc`; CP3 is `55b5313`; CP4 is `6f69310`; CP5 is `3259c42`. The [CP5 report](acceptance/road-v0-7/cp5-integration/report.md) records the complete Python/Vue/Rust gate plus isolated current-source Tauri normal, native dirty-confirmation, Index, repair, `commit_unknown`, restart/no-replay and clean-exit paths on `test/cp5-v07-integration`. No unresolved P0/P1 was found. Product acceptance remains unfinished; real-Vault work, push, tag, closeout, and release remain unauthorized. |
...
```text
current: Road v0.6 Paper v4 / Index v4 / protocol v2 remains the accepted product baseline
candidate: Road v0.7 production composition and cross-stack synthetic platform Gate passed CP5
next:    CP6 branches from the passed CP5 checkpoint for separately authorized first-author acceptance
```
...
Python `265`, Vitest `77`, Rust `12` and every build/format/docs/bundle gate passed. An isolated fake Home and synthetic Vault completed startup, two-page save, Library/Vault, native dirty confirmation, `920×680`, Index, repair and `commit_unknown` no-replay paths in Tauri; `library.branch=1`, exactly two Papers, normal sidecar SHA restoration and zero recorded child residue were verified. This is synthetic engineering/platform evidence, not real-provider or CP6 acceptance; WebView console and native IME were not separately claimed.
...
- **Views:** [design](design/design.html), [interaction](design/interaction.html), and [architecture](architecture/architecture.html) show accepted v0.6 current state beside the implemented v0.7 candidate. CP2 Shell, CP3 Paper, CP4 Library/Vault and CP5 integration evidence live in their reports; the transition pages do not prove CP6.
...
Road v0.4, Road v0.5, and Road v0.6 are complete. Road v0.7 CP0–CP5 passed under advance YOLO; the production Shell, Paper, Library, Vault and recovery surfaces are implemented and the synthetic cross-stack platform Gate passed, while CP6 first-author product acceptance remains unclaimed and real Vault use remains separately unauthorized. Road v0.6 CP7 first-author product acceptance passed after one native-confirmation P1 was fixed and retested; normal data/runtime contracts remain Paper v4/Index v4/protocol v2. The authorized existing Vault was already v4, so no real-Vault migration occurred. The closeout snapshot binds CP7 checkpoint `18a1024`; tag, push, signing, packaging, release and broader compatibility remain unperformed and unclaimed.
