...
| Road v0.8 | Direction documented; CP0 seed not approved; not started | On 2026-08-30 the developer approved the replacement [cross-platform direction and documentation draft](../PLAN_road_v0_8.md), not an executable plan seed or CP0 branch. The close-guard deviation must first receive an independent narrow fix and focused re-verification, then the plan needs explicit seed approval. No mobile Rust Core, bilingual UI, iCloud capability, release or Alpha work has started; Road v0.7 supplies the accepted baseline. After v0.8 passes, v0.9 owns the iOS/macOS first Alpha and promotion Gate, v0.10 owns Android and the second Alpha, and only both rounds may seed a Windows Road. |
...
next:      independently fix and reverify the close guard; only then request explicit approval to seed Road v0.8 CP0 from the documented replacement plan
```
