...
| Road v0.4 | CP7 complete; CP8 next | Vue routes between Paper and a read-only Summary-first Flashcard projection. Focused tests and isolated browser QA cover selection, navigation, reset, Summary context, and return-to-Paper; the developer accepted CP7 on 2026-07-25. |
...
Road v0.4 CP0 is committed at `8407941` on `codex/road-v04-cp0`; CP1 at `b718ed8`; CP2 at `42dfa68`; CP3 at `e72f155`; CP4 at `0d86847`; CP5 at `f19da31`; and CP6 at `b52622f`. CP7 is accepted on `codex/road-v04-cp7`; its checkpoint commit hash is reported in the handoff. Each checkpoint uses its own branch created from the previous accepted checkpoint.
...
- [`FlashcardView.vue`](../frontend/src/FlashcardView.vue) consumes only
  `flashcard.open` DTOs. Selection, list/arrow/jump navigation, Summary context,
  and position stay in Vue memory; every open and Paper switch resets to page 1.
- App routing carries the selected Vault-relative path back to Paper. Leaving a
  dirty Paper still requires the existing discard confirmation.
- Vitest completed with `21 passed`; the production Vite build and
  `git diff --check` passed.
- Isolated browser QA at 1220×780, 768×800, and 375×812 covered Summary-first,
  list/arrow navigation, Summary context, Paper switch/reset, and return to the
  selected Paper. No overflow, duplicate ID, unlabeled control, unnamed button,
  or console warning/error was found.
- No real Vault, native `.app`, dependency, Python, protocol, or Rust code was
  touched. No screenshot baseline exists, so visual regression is inconclusive.
...
1. Commit the accepted CP7 checkpoint, then create `codex/road-v04-cp8`
   from that exact HEAD.
2. Implement the CP8 Library read slice without pulling in CP9 mutations.
