...
| Post-Road developer tooling | TUI implemented; technical manuals current | `./dev` provides owned-process launch/log control and local document browsing; the [manual index](manual/README.md) now routes to TUI, stack, testing/evidence, and build/packaging guides. The ignored context pack is a disposable agent handoff artifact. None changes product UI, protocol, Vault behavior, or acceptance. |
| Road v0.8 | Plan draft recorded; not started | The developer authorized the pre-start [plan draft](../PLAN_road_v0_8.md) on 2026-08-26. Its earliest-date condition was met on 2026-08-28, but explicit plan/seed approval remains absent. The close-guard implementation deviation recorded below must first receive a fix assignment and focused re-verification. No Road v0.8 checkpoint, release build, signing, package, distribution, remote write or release action has started; Road v0.7 supplies the accepted baseline. |
...
next:      assign and reverify the close-guard fix; the date condition is met, but Road v0.8 CP0 still needs explicit plan/seed approval
```

**2026-08-26 implementation deviation:** The accepted contract keeps App-root pending durable intent under normal-close protection until reconciliation or explicit abandonment. Source stores the intent in `App.vue`, but `PaperView.vue` owns `registerWindowCloseGuard()`; runtime-blocked screens therefore unmount the only Tauri guard, and Library/Vault durable intents have no equivalent App-root guard. This audit changed no production code or real Vault. Road v0.8 CP0 remains blocked until a narrow fix is assigned and focused Paper/Library/Vault/blocked-screen tests reverify it. See [SPEC](SPEC.md), the [Paper v4 design](design/road-v0-6-paper-v4-design.md), and the accepted [App Shell design](design/road-v0-7-app-shell-design.md); source and tests below remain runtime fact.
...
- **Authority:** [SPEC](SPEC.md) owns the accepted CP8 product boundary; the [Paper v4 design](design/road-v0-6-paper-v4-design.md) owns unchanged grammar/protocol; the [App Shell design](design/road-v0-7-app-shell-design.md) owns CP6 history and the accepted CP7/CP8 matrices; the Chinese [plan](archive/road-v0-7/PLAN_road_v0_7.md) owns Gate order. [RULES](RULES.md) owns implementation discipline; [AGENTS](../AGENTS.md) owns agent procedure. Runtime facts come from [`src/`](../src/) and [`tests/`](../tests/).
...
Road v0.4–v0.7 are complete. Road v0.7 ends at CP8 checkpoint `2f03aee`; its independent [snapshot](archive/snapshots/road-v0-7.html) records scope, evidence, QA, omissions and risks. Paper v4/Index v4/protocol v2 and the complete-folder lifecycle remain unchanged. CP6 completed its bounded real-v4-Vault Gate and consumed that authorization; no later real-Vault operation occurred. Push, tag, signing, packaging and release remain unperformed. Road v0.8 is not started: its [pre-start plan draft](../PLAN_road_v0_8.md) is recorded and the earliest-date condition is now met, but explicit plan/seed approval is absent and the current close-guard deviation still needs an assigned fix path plus focused re-verification.
