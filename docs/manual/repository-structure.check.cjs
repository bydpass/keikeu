...
assert.deepEqual(search('MARKDOWN_IO.PY'), ['src/keikeu_core/markdown_io.py']);
const nested = document.querySelector('[data-path="src/keikeu_core"]');
assert.equal(nested.hidden, false);
assert.equal(nested.querySelector('details').open, true);
assert.deepEqual(search('frontend/src/'), paths.filter(value => value.startsWith('frontend/src/')));
