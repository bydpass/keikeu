...
> **预启动状态（2026-09-04 文档同步）：** [Road v0.8](../../../PLAN_road_v0_8.md) 软件 CP0 seed 未批准；
> CP1 的获准开发探针与签名验证不等于发布。候选后须独立完成桌面对等与 Python 产品运行时退役，
> Road v0.9 才执行最终分发验证和首轮外部 Alpha；发布合同尚未批准。
...
先确认独立统一核心收口已通过，分发候选不再依赖 Python 产品运行时；再按以下顺序证明：
