...
- 2026-09-21 开发者授权：Road 一旦宣布完成，同轮将已验证的提交合入主工作树 `/Users/chenxi/kits/keikeu`（不等同于切换到名为 main 的分支），优先快进；完成收口文档后移除该 Road 及已被接续的旧工作树。先核对提交可达性、未提交改动、运行进程和被忽略文件；唯一内容与必要证据迁入主工作树的既有对应位置并核对后，才移除副本。不能因清理丢弃作者内容或未合入工作。
- 收口时通知 `dsh` 同步最终状态，并按主代理核实的精确清单清理工作树残留、重复 `.bak` 与可重建缓存；禁止自行扩大路径、读取或上传私有正文及凭据。无害但无法判明归属的内容保留并报告；不为清理另造散落的备份工作树。清理后核对 `git worktree list`、主工作树状态及交接路由；报告实际清除项和有理由保留项。
- 交付报告 worktree、分支、HEAD、暂存／提交／推送状态及下一步。
- 启用外部编码助手时使用用户可见的交互界面：Claude Code 在可见终端运行 `claude`；dsh 的外包流程为打开 `dsh web` → 在 `keikeu` 项目工作区内启动 session → 下发含范围、权限与验收要求的外包任务 → 等待结果返回 → 核对该 session 的执行轨迹及实际产物后汇报。检查工具调用、访问与修改范围、失败或权限阻点、实际验证及其输出是否支持最终结论；不能只看答复摘要，也不能以已发送代替已完成。轨迹不可用或证据不足时明确报告未核实项。除非用户另行要求，不使用隐藏的 headless 会话。核对模型时同时检查用户覆盖设置和当前会话选择，不能只凭基础默认别名认定版本。

## 8. Evidence

- 将工程完成、文件服务 smoke、产品接受和 Road 归档分别判定；每项结论注明对应源码／包、操作、结果、日期及缺项。历史检查以历史日期引用，本轮检查只列实际执行结果。
- 工程 Gate 可按既有提前 YOLO 收口；真实设备和 provider 判据由对应实际观察满足。旧 v08 CP5 的 B01–B16 结果保留原证据边界，B14 按 2026-09-20 开发者决定关闭且不再补验，不作为新 Road 的继承阻点；新原生版本按实际实现定义验收。v09 按自身工程计划接受。
