...
最终检查点 `c615ce5` 已提交；其 75 个构建源码文件逐项匹配实际冒烟包的源码清单。收口分支 `codex/road-v09-closeout` 仅更新完成状态与[施工快照](../../archive/snapshots/road-v09.html)，不改产品源码。原规划与旧候选工作区仍干净，CLI 隔离工作区保留已集成的两行补丁。本轮没有推送或发布。最终 CONTEXT 路由在该文档提交后生成并核对，不进入版本控制。

收口文档检查 87 active／14 required 通过，快照本地链接逐项有效；目录页 294 路径及交互检查通过，差异检查通过。本次纯文档收口未重跑应用测试，CP4 的源码和包未改。

## 收口后整合与清理（2026-09-21）

开发者明确要求校正措辞、提交并修剪工作树；今后每个 Road 宣布完成后都须同轮合回主工作树 `keikeu`，通知 dsh 清理确认的残留。`87a33a8` 保存 B14 关闭决定、SPEC 历史证据措辞及这一流程，主工作树已从 `670142d` 快进至该提交。这里的主工作树指路径，不要求切换为名为 main 的分支；当前分支仍为 `codex/docs-v08-visible-road`。

清理前核对提交可达性、进程、未提交改动和忽略文件。v09 CLI 的唯一两行改动与合入文件逐字节一致；v08 与 v09 执行树均无未合入改动。历史证据、候选包、Apple 生成工程和迁移恢复数据以同文件系统重命名集中进入 `tests/test-vault/history/`，前后 inode 一致；v09 运行回执移至主工作树 `tests/test-vault/v09/`。不按 `.bak` 或 `K-*.md` 名称推断内容可删。

dsh 已完成元数据盘点并收到精确清理白名单，其执行在删除前被自身沙箱的 `ps` 权限阻止，零删除。主代理在已有授权下原样执行带前置校验的脚本，完成移除：`keikeu-v08-execution`、`keikeu-v09-execution`、`keikeu-v09-cli`，以及主工作树旧 `frontend/`、只剩字节码的 `src/`、空 `build/`、已迁出唯一证据的 `retired-cp3-worktree-20260907` 外壳。两个顶层 bak／backups 数据目录迁入统一历史区后原路径消失，未删除作者稿或恢复副本。

最终 `git worktree list` 仅有 `/Users/chenxi/kits/keikeu`，同级无其他 keikeu 工作目录；目录总占用由约 21.6 GB 降至 4.3 GB（目录统计，不声称物理磁盘回收量）。分支和提交历史保留，未推送。

主工作树复用已安装前端依赖与已验证 sidecar，并重新安装本目录的 Python editable 包；核对两个 Python 包均从主工作树 `apps/desktop/python/` 导入。合入后实际 Python 302 项、前端 142 项通过；文档 87/14 与差异检查通过。Rust 与原生 Mac 冒烟沿用 CP4 同源码证据，本轮未重跑；没有改签名、账号、存储设置或日常 Vault。清理回执与 dsh 会话回执在忽略的 `tests/test-vault/road-cleanup-20260921/`，最终交接路由在本次提交后刷新。
