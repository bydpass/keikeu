...
> 本次只交付计划与辅助图；新版工程尚未启动，不恢复旧的自动推进任务。
...
**当前游标：计划重写。** 每次交付只更新下面这一行，不能用“代码已写”冒充设备验收。

| 本轮状态 | 当前修改 | 已确认 | 下一可见成果 | 待人工 |
| --- | --- | --- | --- | --- |
| 规划中 | 计划、辅助图、状态说明 | 基线与关闭保护 | CP0 的接口及写入责任表 | 本次无设备操作 |
...
开发者批准新版并明确开始工程后进入 CP0。已授权范围内的检查、修复和构建连续执行；
阶段是否通过由本计划的证据决定，不逐按钮请求确认。此次重写不自动启动产品迁移。
