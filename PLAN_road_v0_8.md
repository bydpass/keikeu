...
> 2026-09-20 当前决定：v08 已中断并转交，未获最终接受；v09 整顿工作目录与执行体系，v09.01 直接替换为 iOS SwiftUI。移动端采用原生组件并尽可能复用 Rust 内核；Mac 日后重构，Windows 保留现有技术结构。本轮 v09 CP0–CP4 已获全计划 YOLO，先重排目录并独立纯规则核心。当前方向与转交清单见 [Road v09 规划](docs/road-v09.md)；下文 v08 目标和候选门槛保留作历史对照，不再驱动旧 iOS 路线开工或收尾。外部 Alpha 移出 v09，后续版本编号待定。
...
- 原生冲突和 provider 改名副本均须支持发现、显示、完整保全、导出与明确恢复。按开发者 2026-09-07 批准的 B10 调整，同 code 离线新建按 provider 实际结果验收；原生冲突保全由 B09／B11 独立证明，不宣称同 code 新建已产生原生冲突。
...
当前同源码候选为 `83c4f60`，交付位于 `system-target-fix/`；Mac 与 iPhone 均已安装并确认普通界面启动。共享系统目标错误解析修复通过 Rust 21 项检查。手机未闭合 ASCII 引号两页重启恢复已补齐；真实保存／恢复中断核对及本机单稿／目录生命周期已取得补充证据。手机三种导出字节与多冲突保全中断已补齐；下载切换与本地回归已补齐，前端全量 142 项通过。同编号新建两轮产生自动改名副本。开发者已批准 B10 按 provider 实际结果验收，原生冲突保全由 B09／B11 独立证明；B10 按修订判据通过，不宣称同编号新建原生冲突实测通过。当前仅 B14 账号／容器不可用矩阵及最终候选接受待完成，CP5 不标通过。下文保留此前阶段记录，最新逐项状态以个人真机测试单为准。
...
前端 141 项通过，双端新候选在线连续保存通过。原始错误、保全副本和时间证据已记录；新版离线编辑复测正常，新增韩文字段已在 Mac 重连读回。Mac 未闭合 Tags 两页草稿关闭重开恢复、原文导出字节核对及取消通过；既有草稿离线首次保存及 Mac 读回通过；真实双端并发产生原生冲突，两份原字节保全及原生导出核对通过。全程断网新建、同 code 冲突、多冲突中断等其余必需矩阵仍待验。

原生恢复测试发现共享会话方法列表漏列恢复命令，导致 `unsupported_method`；已补齐入口，并将现有恢复回归改为经过会话请求。修复前复现、修复后 Rust 20 项通过。双端重建安装包，Mac 原生恢复、恢复前保全、过期确认拒绝覆盖及损坏副本只导出已验证；手机已安装新版，锁屏阻止自动启动，界面复测待解锁。该轮候选为 `promotion-fix/`；后续已由上方当前候选替代。

新版手机普通启动及恢复稿读回已验证。手机全程断网新建 `K-20260907-003`、保存及联网后 Mac 读回通过；双端同编号新建实际产生 provider 自动改名副本，两份原字节保全与原生导出核对通过。同编号新建的原生版本冲突分支、多冲突中断等仍待验。
