...
原生恢复测试发现共享会话方法列表漏列恢复命令，导致 `unsupported_method`；已补齐入口，并将现有恢复回归改为经过会话请求。修复前复现、修复后 Rust 20 项通过。双端重建安装包，Mac 原生恢复、恢复前保全、过期确认拒绝覆盖及损坏副本只导出已验证；手机已安装新版，锁屏阻止自动启动，界面复测待解锁。当前候选为 `promotion-fix/`，CP5 仍待剩余必需证据。
