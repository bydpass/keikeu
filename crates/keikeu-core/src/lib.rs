...
mod codec;
mod unicode_data;
pub use codec::{code_sequence, parse, render, validate_code, Page, Paper};
...
#[cfg(test)]
mod tests {
    use super::comparison_key;

    #[test]
    fn unicode_comparison_preserves_existing_semantics() {
        assert_eq!(comparison_key("Straße"), comparison_key("STRASSE"));
        assert_eq!(comparison_key("e\u{301}"), comparison_key("É"));
        assert_eq!(comparison_key("Ა"), "ა");
        assert_eq!(comparison_key("한글"), "한글");
    }
}
